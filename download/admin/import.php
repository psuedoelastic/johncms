...
echo '<div class="phdr">'.$lng_dl['import_file'].'</div>';
$cat = intval($_GET['cat']);
...
if (!isset($_POST['type_import'])) {
    ...
    echo '<div class="rmenu">'.$lng_dl['warn_import'].'</div>';
...
<div class="menu"><b>*</b> '.$lng_dl['file_url'].':<br/><input type="text" name="url_file[' .
            $i . ']" value="http://"/></div>
<div class="menu"><b>*</b> '.$lng_dl['name'].':<br/><input type="text" name="name[' .
            $i . ']" value=""/></div>';

        // Скриншот //
        if (in_array('urlscreen', $arr_input))
            echo '<div class="menu">'.$lng_dl['screen_url'].':<br/><input type="text" name="urlscreen[' .
                $i . ']" value=""/></div>';

        // Имя для фтп //
        if (in_array('ftpname', $arr_input))
            echo '<div class="menu">'.$lng_dl['name_in_file_system'].':<br/><input type="text" name="ftp[' .
                $i . ']" value=""/></div>';
...
<select name="namekat[' . $i . ']" class="textbox">';
    ...
    for ($i = 1; $i <= $_POST['col_files']; $i++) {
        ...
        if ($url) {
            if (mb_substr($url, 0, 7) !== 'http://')
                $error[] = $lng_dl['incorrect_url'].': ' . $url;
            else
                $url = str_replace('http://', '', $url);
        }

        if (empty($url)) {
            $error[] = $lng_dl['empty_url'];
        }
        ...
        `user_id` = '" . $user_id . "';");
                ...
                echo '<div class="menu"><a href="admin.php?act=file&amp;view=' . ($more_file > 0 ?
                    $more_file : $rid) . '">'.$lng_dl['back_to_file'].'</a> | <a href="admin.php?act=folder&amp;cat=' .
                    $catid . '">'.$lng_dl['to_section'].'</a></div>';
            }
            else
            {
    ...
    echo '<div class="gmenu">'.$lng_dl['import_success'].'</div>';
