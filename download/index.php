...
//TODO: ужесточить проверку $_GET['sort']. Сделать массив с возможными значениями и проверять полученные данные по этому массиву.
...
if (in_array($act, $do)) {
    ...
    while ($dirid != '0' && $dirid != "") {
        $req = mysql_query("SELECT * FROM `downpath` WHERE `id` = '" . $dirid . "' LIMIT 1");
    ...
    $textl = $lng['downloads'] . ' ' . strip_tags($valueeq . $cdir) . '';
